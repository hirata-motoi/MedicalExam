package Test::MedicalExam::Dumper;
use strict;
use warnings;
use File::Temp;
...
use Test::MedicalExam;
use Test::MedicalExam::DBHResolver;
...
sub new {
    ...
    Test::MedicalExam::DBHResolver->load($config || __config_file($service));

    bless {
        region   => $region,
        dbh      => Test::MedicalExam::DBHResolver->resolver($name),
        database => Test::MedicalExam::DBHResolver->database($name),
        tables   => $tables  || [],
        data     => $fixture || [],
        schema   => "",
        fixture  => {},
...
sub __config_file {
    ...
    sprintf("%s/config/db/dev.yaml", MedicalExam->base_dir, $env);
}
...
sub dump {
    ...
    my $db = $self->{database};
    my $module_name = Test::MedicalExam->module_name('schema', $self->{region}, $db);
    my $path = Test::MedicalExam->module_path('schema', $self->{region}, $db);
    my $text = __module_text($module_name, $self->{schema});
    __create_module($path, $text);

    for my $tbl (keys %{ $self->{fixture} }) {
        my $module_name = Test::MedicalExam->module_name('fixture', $self->{region}, $db, $tbl);
        my $path = Test::MedicalExam->module_path('fixture', $self->{region}, $db, $tbl);
...
use strict;
use warnings;
use parent qw/Test::MedicalExam::Reader/;
...
sub _create_teng_schema {
    ...
    my $module_name = Test::MedicalExam->module_name(@ns);
    my $module_path = Test::MedicalExam->module_path(@ns);
...
MODULE
    ...
    my $schema_path = Test::MedicalExam->module_path(@ns, 'Schema');
