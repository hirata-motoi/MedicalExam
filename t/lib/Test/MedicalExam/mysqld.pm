package Test::MedicalExam::mysqld;
use strict;
use warnings;
use JSON;
use File::Spec;
...
our $MY_CNF    = $ENV{PROTOSERVER_MY_CNF} || Test::MedicalExam->base_dir . '/my.cnf';
...
sub create_database {
    ...
    Test::MedicalExam->diag("Created database: $dbname");
...
sub do_sql {
    ...
    Test::MedicalExam->diag("Loading SQL to $dbname");

    my $dbh = $self->dbh($dbname);
    chomp $sql;
    map { $dbh->do($_) } grep { $_ =~ /.+/ } split(/;/, $sql);

    Test::MedicalExam->diag("Loaded SQL to $dbname");
}

sub _create_q4m_functions {
    my ($self, $dbh, $dbname) = @_;

    Test::MedicalExam->diag("Install Q4M plugin: $dbname");
...
sub cleanup {
    ...
    for my $db (@$rows) {
        ...
        Test::MedicalExam->diag("Removed database: $db");
    }
}
