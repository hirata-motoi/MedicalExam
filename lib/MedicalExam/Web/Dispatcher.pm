package MedicalExam::Web::Dispatcher;
use strict;
use warnings;
use utf8;

use MedicalExam::Web::Router;
...
my $router = MedicalExam::Web::Router->new;
{
    for my $pl (dir(MedicalExam->base_dir . "/route")->children) {
...
{ # for access log
    my $fh = File::Stamped->new(pattern => MedicalExam->base_dir . "/log/access.log.%Y%m%d");
...
sub dispatch {
    ...
    my $web_root = $ENV{DEBUG_WEB_ROOT} || ref MedicalExam->context;
    my $req      = $c->request;
...
sub _filter {
    ...
    for my $filter ( @$filters ) {
        ...
        my $res_type = ref $res eq 'Amon2::Web::Response'  ? FILTER_RESULT_RESPONSE  :
                       ref $res eq 'MedicalExam::FormValidator' ? FILTER_RESULT_VALIDATOR : '';
        $results->{$res_type} = $res;
    }

    $results->{ FILTER_RESULT_VALIDATOR() } ||= MedicalExam::FormValidator->new($c->req);
    return $results;
}
