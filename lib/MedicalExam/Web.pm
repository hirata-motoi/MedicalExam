package MedicalExam::Web;
use strict;
use warnings;
use utf8;
use parent qw/MedicalExam Amon2::Web/;
use File::Spec;
use Log::Minimal;
use Carp;
use MedicalExam::Web::Root;
...
__PACKAGE__->load_plugins(
    'Web::FillInFormLite',
    'Web::JSON',
    '+MedicalExam::Web::Plugin::Session',
    'Web::Stash' => +{
       autorender => 1,
    },
);
...
# dispatcher
use MedicalExam::Web::Dispatcher;
sub dispatch {
    return ( MedicalExam::Web::Dispatcher->dispatch($_[0]) or __exception("response is not generated") );
}
...
# setup view
use MedicalExam::Web::View;
{
    sub create_view {
        my $view = MedicalExam::Web::View->make_instance(__PACKAGE__);
        no warnings 'redefine';
        *MedicalExam::Web::create_view = sub { $view }; # Class cache.
        $view
    }
}
...
__PACKAGE__->add_trigger(
    BEFORE_DISPATCH => sub {
        ...
        my $session_not_required_paths = MedicalExam::Common->config->{session_not_required_paths} || [];
        ...
        my $base_info = MedicalExam::Web::Root->new->certify($session_id);
        ...
        if ($c->stash->{session_have_to_update}) {
            ...
            my $new_session_id = MedicalExam::Web::Root->new->update_session($c->stash->{user_id});
