package MedicalExam::Model::ImageFile;
...
sub factory {
    ...
    croak sprintf('Invalid file type : %s', $params->{path})
        if ! grep { $format eq $_ } @{ MedicalExam::Common->config->{allowed_image_format} };

    my $class_name = sprintf 'MedicalExam::Model::ImageFile::%s', camelize($format);
